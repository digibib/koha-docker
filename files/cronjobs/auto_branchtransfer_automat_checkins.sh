...
# sets `datearrived` to NOW if there are no reserves on biblio
# OR all reservations on biblio are suspended AND tobranch matches automat branch
#
...
cat <<-EOF | koha-mysql $(koha-list --enabled) --default-character-set=utf8
  ...
  JOIN items i ON (i.itemnumber=bt.itemnumber)
      SET bt.datearrived = NOW(), i.holdingbranch = aut.branch
    WHERE bt.tobranch = aut.branch
    AND bt.datearrived IS NULL
    AND i.biblionumber NOT IN (SELECT biblionumber from reserves WHERE suspend = 0) ;
EOF
