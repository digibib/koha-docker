...
my $opac_base_url = '/usr/share/koha/opac';
my $intranet_base_url = '/usr/share/koha/intranet';
...
if ( $prefs_count or $patrons_count ) {
    say "Database is not empty! Running Update instead...";
	my $failure = update_database();
	if ($failure) {
		die "updatedatabase.pl process failed: $failure";
	} else {
		say "Data update succeeded";
		exit 0;
	}
...
my @marc21_sample_files_mandatory  = ( glob $root . "/installer/data/mysql/$lang/marcflavour/marc21/mandatory/*.sql" );
my @marc21_sample_files_optional   = ( glob $root . "/installer/data/mysql/$lang/marcflavour/marc21/optional/*.sql" );
my @unimarc_sample_files_mandatory = ( glob $root . "/installer/data/mysql/$lang/marcflavour/unimarc/mandatory/*.sql" );
...
sub initialize_data {
    ...
    if ( $marcflavour eq 'UNIMARC' ) {
        ...
        if ($optional) {
		    for my $f (@marc21_sample_files_optional) {
		        execute_sqlfile($f);
		    }
		}
    }
